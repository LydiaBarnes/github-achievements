# Github Achievements Prototype

This is a Hackathon project I made in 2 days. My goal was to prototype out what a game-like achievements system might look like for a company (usually tied to Github organization) that wants to celebrate everyday wins in its engineering team.

It is a proof of concept, so I only created a few achievement types as a demonstration of whats possible. For example: number of repos contributed to, number of PRs approved, code additions/deletions, etc. You can have badges that stick to each user's profile for large milestones, and a feed of achievements. I chose to do a style that rewards actual output rather than competing employees against each other. The idea is an achievement sticks around, but being "1st" can go away, so thats not as positive.

Since I made this in only 2 days, there is a ton left to do. (I would NOT use this as production code!!! Created during a Hackathon, so very hacky!)
* Use Apollo instead of axios, mostly for caching and some data management. (right now there is no caching so its a bad idea to click through too fast, Github api will hate you)
* Add actual user authentication steps so you don't need an environment access token
* Add ability to change organization (I hardcoded Github org for demo)
* General code cleanup

Since again, this was a Hackathon project and proof of concept, I may or may not ever update this project again. Who knows? :shrug-emoji:


## How to demo
Follow the steps for running a React app locally in the generated readme below.
You will also need to create a `.env` file with your own personal Githib access token. Look at `.sample-env` as a guide. (Never commit this!)
